		...
		for (int i = 0; i + 2 < a.length; i++) {
			...
			while (startIndex < endIndex) {
				if (a[startIndex] + a[endIndex] == targetSum) {
					result.add(Arrays.asList(a[i], a[startIndex], a[endIndex]));

					do { // loop forward until we encounter a different element
						startIndex++;
					} while (startIndex < endIndex && a[startIndex] == a[startIndex - 1]);

					do { // loop backward until we encounter a different element
						endIndex--;
					} while (startIndex < endIndex && a[endIndex] == a[endIndex + 1]);

				} else if (a[startIndex] + a[endIndex] > targetSum) { // sum is too high: decrement end
					endIndex--;
				} else { // sum is too low: increment start
					startIndex++;
				}
			}
		}
