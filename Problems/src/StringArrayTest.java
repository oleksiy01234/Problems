...
class StringArrayTest {
  ...
  static int anagram(String s) {
    ...
    return makeAnagrams(s.substring(0, s.length() / 2), s.substring(s.length() / 2));
  }
  ...
  static int smallestNonConstructableValue(int[] a) {
    Arrays.sort(a);
    int max = 0;
    for (int i : a) {
      if (i > max) {
  ...
  static boolean rotateInPlace(int[][] a) {
    ...
    for (int layer = 0; layer < n / 2; layer++) {
      ...
      for (int i = layer; i < last; i++) {
        int offset = i - layer;
        int top = a[layer][i]; // save top
        a[layer][i] = a[last - offset][layer]; // top = left
        a[last - offset][layer] = a[last][last - offset]; // left = bottom
